<? #V A1.5.1
class user_comment_feed extends base{
  ...
  function build( $id=false ){
	...
	$this->_settings['db_id'][] = 'comment_id';
...
WHERE comment.user_id_er = user.user_id
...
order by dt desc limit 0,10
	*/
    $this->_settings['baseQuery']= "SELECT * FROM comment, user
									LEFT JOIN image on user.avatar = image.image_id
									WHERE comment.user_id_er = user.user_id
									AND user.user_id in
